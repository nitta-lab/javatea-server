...
public class UserTimetableResource {
    ...
    public Response getYears(@PathParam("uid") String uid, @QueryParam("token") String token) {
        ...
        // 年度取得
        ArrayList<Integer> years = timetableRepository.getYears(uid);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getLectures(@PathParam("uid") String uid, @PathParam("year") int year, @QueryParam("token") String token) {
        // ユーザの存在チェック
        User user = userRepository.getUser(uid);
        if (uid == null || user == null) {
            throw new WebApplicationException(
